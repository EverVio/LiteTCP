...
if os.path.exists(build_bin):
    ...
    os.environ["PATH"] = os.path.abspath("bin") + os.path.pathsep + os.environ.get("PATH", "")
...
def run_reliable_tests(drop_rate=0.05, delay_ms=10, payload_size=5242880):
    ...
    try:
        ...
        time.sleep(0.5)
        client = run_case("test_reliable", "--client", 2, drop_rate, delay_ms, payload_size)
        ...
        chk_client = "test_results/checksum_client.txt" if os.path.exists("test_results/checksum_client.txt") else "checksum_client.txt"
        chk_server = "test_results/checksum_server.txt" if os.path.exists("test_results/checksum_server.txt") else "checksum_server.txt"
...
def main():
    ...
    parser.add_argument("target", choices=["connect", "close", "reliable", "all"], default="all", nargs="?", help="Test target")
    parser.add_argument("--drop", type=float, default=0.02, help="Packet drop rate (0.0 to 1.0) for reliable test")
    parser.add_argument("--delay", type=int, default=20, help="Packet delay in ms for reliable test")
    parser.add_argument("--size", type=int, default=5242880, help="Payload size in bytes for reliable test (default 5MB)")
    ...
    try:
        if args.target == "connect":
            ...
            run_reliable_tests(drop_rate=args.drop, delay_ms=args.delay, payload_size=args.size)
