...
LiteTCP 是一个基于 **C++17 编程标准** 实现的轻量级用户态 TCP 框架。该框架通过运行在本地 UDP 媒介之上，简化模拟了经典 TCP 协议的核心要素，包括三次握手与四次挥手连接管理、基于滑动窗口的流量控制、重传计时器管理，以及对类 NewReno 拥塞控制快速恢复机制的基本逻辑模拟。
...
* **拥塞控制模拟 (类 NewReno)**：在用户态模拟了由慢启动、拥塞避免、快速重传与快速恢复构成的控制环路。快速恢复模块简化模拟了部分确认与完全确认的差异化重传和窗口扣减逻辑。
...
2. **简化类 NewReno 拥塞算法与滑动窗口的结合**：
   在 [TcpSocket_State.cpp](src/TcpSocket_State.cpp) 中，发送数据的上限受限于 `std::min(cwnd, peer_rwnd)`。在收到连续 3 个冗余 ACK 时，程序会减半 `ssthresh` 并进入快速恢复；在快速恢复期间简化模拟了 NewReno 的处理方式，即区分处理部分确认（Partial ACK，触发最老包重传并扣减拥塞窗口）与完全确认（Full ACK，退出快速恢复状态），展示了经典的窗口拥塞负反馈调整机制。
...
3. **拥塞控制算法简化**：本框架的拥塞控制仅是对慢启动、拥塞避免、快速重传与快速恢复核心逻辑的简化模拟，并未完整且严密地实现 NewReno 标准规范中的各种 cwnd 计算限制、限制传输和 RTT 去二义性等机制，不具备真实复杂网络的吞吐优化能力。
