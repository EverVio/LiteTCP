...
def plot_advanced_metrics():
    ...
    # ------------------ 图 1: 拥塞控制全景与微观图 ------------------
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(11, 7), gridspec_kw={'height_ratios': [1.2, 1]})
    
    # 绘制发送端 cwnd 与慢启动阈值 ssthresh 的全景趋势图。
    ax1.plot(timestamps, cwnd, label="cwnd (Congestion Window)", color="#1f77b4", linewidth=1.5, zorder=2)
    ax1.plot(timestamps, ssthresh, label="ssthresh", color="#ff7f0e", linestyle="--", linewidth=1.5, zorder=1)
    ...
    if rto_t:
        ax1.scatter(rto_t, rto_v, color="#d62728", marker="x", s=30, label="RTO Timeout", zorder=3)
    if fr_t:
        ax1.scatter(fr_t, fr_v, color="#9467bd", marker="o", s=25, label="Fast Retransmit", zorder=3)

    ax1.set_title("LiteTCP Sender Congestion Control Status (TCP Reno)", fontsize=13, fontweight="bold", pad=12)
    ...
    ax2.set_title(f"Microscopic View: First {zoom_limit} Seconds Dynamic", fontsize=11, fontweight="bold", style="italic")
    ...
    plt.plot(timestamps, rwnd, label="rwnd (Receiver Window)", color="#2ca02c", linewidth=2.0)
    plt.fill_between(timestamps, rwnd, color="#2ca02c", alpha=0.08)
    
    plt.title("LiteTCP Receiver Flow Control Status (rwnd)", fontsize=12, fontweight="bold", pad=10)
